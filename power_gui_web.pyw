...
class PowerMonitorApp(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"⚡ Power Monitor (V46 Windows)")
        ...
        self.lbl_title = ctk.CTkLabel(self.frame_header, text="SYSTEM POWER DRAW", font=("Roboto Medium", 12),
        ...
        ctk.CTkLabel(self.frame_stats, text="TIMELINE", font=h_font, text_color=COLOR_TEXT_SUB).grid(row=0, column=0,
                                                                                                     pady=10,
                                                                                                     sticky="w")
        ctk.CTkLabel(self.frame_stats, text="COST (EGP)", font=h_font, text_color=COLOR_TEXT_SUB).grid(row=0, column=1,
                                                                                                       pady=10,
                                                                                                       sticky="e")
        ctk.CTkLabel(self.frame_stats, text="ENERGY", font=h_font, text_color=COLOR_TEXT_SUB).grid(row=0, column=2,
                                                                                                   pady=10, sticky="e")
        ctk.CTkLabel(self.frame_stats, text="DURATION", font=h_font, text_color=COLOR_TEXT_SUB).grid(row=0, column=3,
                                                                                                     pady=10,
                                                                                                     sticky="e")
        ...
        ctk.CTkFrame(self.frame_stats, height=2, fg_color="#404040").grid(row=4, column=0, columnspan=4, sticky="ew",
                                                                          pady=10)

        # Config
        ctk.CTkLabel(self.frame_stats, text="LIVE CONFIGURATION", font=("Arial", 11, "bold"), text_color="gray").grid(
            row=5, column=0, pady=(5, 0), sticky="w")
        ...
        self.btn_apply = ctk.CTkButton(self.frame_config, text="Apply", width=50, height=25, fg_color="#404040",
                                       hover_color="#606060", command=self.apply_config)
        self.btn_apply.pack(side="left", padx=15)

        ctk.CTkFrame(self.frame_stats, height=2, fg_color="#404040").grid(row=7, column=0, columnspan=4, sticky="ew",
                                                                          pady=10)

        # Calculators
        ctk.CTkLabel(self.frame_stats, text="COST ESTIMATORS", font=("Arial", 11, "bold"), text_color="#E040FB").grid(
            row=8, column=0, pady=(5, 0), sticky="w")
        ...
        ctk.CTkLabel(self.frame_calc1, text="Monthly 24/7:", text_color="#a0a0a0", width=90, anchor="w").pack(
            side="left")
        ...
        self.btn_calc_1 = ctk.CTkButton(self.frame_calc1, text="Calc", width=60, height=25, fg_color="#404040",
                                        hover_color="#606060", command=lambda: self.calculate_custom_cost(1))
        self.btn_calc_1.pack(side="left", padx=10)
        self.lbl_calc_result_1 = ctk.CTkLabel(self.frame_calc1, text="---", font=("Arial", 13, "bold"),
        ...
        ctk.CTkLabel(self.frame_calc2, text="Custom Task:", text_color="#00E5FF", width=90, anchor="w").pack(
            side="left")
        ...
        self.btn_calc_2 = ctk.CTkButton(self.frame_calc2, text="Calc", width=60, height=25, fg_color="#404040",
                                        hover_color="#606060", command=lambda: self.calculate_custom_cost(2))
        self.btn_calc_2.pack(side="left", padx=10)
        self.lbl_calc_result_2 = ctk.CTkLabel(self.frame_calc2, text="---", font=("Arial", 13, "bold"),
                                              text_color=COLOR_TEXT_MAIN)
        ...
        if not ips:
            ...
            for ip in ips:
                ...
                lbl = ctk.CTkLabel(self.frame_footer, text=label_text, text_color=color, font=("Arial", 12, "bold"),
                                   cursor="hand2")
                lbl.pack(pady=2)
    ...
    def detect_cpu_name(self):
        try:
            if platform.system() == "Windows":
                ...
                name = name.replace("Intel(R) Core(TM) ", "").replace("AMD Ryzen ", "Ryzen ").replace(" Processor",
                                                                                                      "").replace(
                    " 12-Core", "")
                return name
    ...
    def get_all_ips(self):
        ip_list = []
        try:
            hostname = socket.gethostname(); ip_list = [ip for ip in socket.gethostbyname_ex(hostname)[2] if
                                                        not ip.startswith("127.")]
        except:
            pass
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM); s.connect(("8.8.8.8", 80)); main_ip = s.getsockname()[
                0]; s.close(); ip_list.insert(0, main_ip) if main_ip not in ip_list else None
        except:
            pass
    ...
    def create_stat_row(self, row_idx, label_text, color):
        ctk.CTkLabel(self.frame_stats, text=f"{label_text}:", font=("Arial", 13), text_color=COLOR_TEXT_MAIN).grid(
            row=row_idx, column=0, pady=5, sticky="w")
    ...
    def load_data(self):
        ...
        default = {"last_date": today_str, "day_kwh": 0.0, "day_cost": 0.0, "day_seconds": 0, "lifetime_kwh": 0.0,
                   "lifetime_cost": 0.0, "lifetime_seconds": 0}
        if os.path.exists(STATE_FILE):
            try:
                with open(STATE_FILE, 'r') as f:
                    data = json.load(f)
                    if "lifetime_seconds" not in data: data.update({"lifetime_seconds": 0, "day_seconds": 0})
                    if data.get("last_date") != today_str: data.update(
                        {"last_date": today_str, "day_kwh": 0.0, "day_cost": 0.0, "day_seconds": 0})
                    return data
            except:
                pass
        return default
    ...
    def init_csv(self):
        headers = ["Timestamp", "Total Watts", "Total Cost", "CPU Temp", "CPU Watts", "CPU Load", "iGPU Temp",
                   "iGPU Watts", "iGPU Load"]
        for i in range(len(self.gpu_data)): headers.extend(
            [f"GPU{i} Temp", f"GPU{i} Watts", f"GPU{i} Load", f"GPU{i} VRAM Used"])
    ...
    def log_to_csv(self, total_w, cpu_t, cpu_w, cpu_l, igpu_t, igpu_w, igpu_l, nv_metrics):
        try:
            row = [datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f"{total_w:.1f}",
            ...
            for metric in nv_metrics:
                row.extend([f"{metric['temp']:.1f}", f"{metric['power']:.1f}", f"{metric['load']:.1f}",
                            f"{metric['vram_used']:.2f}"])
    ...
    def find_sensor_value(self, node, target_names, sensor_type="Power"):
        if isinstance(node, dict):
            if node.get("Type") == sensor_type and any(
                    name.lower() in node.get("Text", "").lower() for name in target_names):
                try:
        ...
        if "Children" in node:
            for child in node["Children"]:
                if child.get("Type") == "Power" and child.get("Text", "") in ["GPU Core", "GPU SoC", "GPU Power"]:
                    try:
                        acc += float(str(child.get("Value", "0")).split()[0])
                    except:
                        pass
    ...
    def get_color(self, temp):
        return "#FF4444" if temp > 80 else ("#ff8c00" if temp > 60 else "#76b900")

    def format_time(self, seconds):
        m, s = divmod(int(seconds), 60); h, m = divmod(m, 60); return f"{h:02d}:{m:02d}:{s:02d}"
        ...
        while self.running:
            ...
            if self.nvml_active:
                for gpu in self.gpu_data:
                    try:
                        ...
                        shared_gpu_list.append(
                            {"name": gpu['short'], "power": w, "temp": t, "load": l, "vram_used": vram_used_gb})
                        if gpu['widget_pwr']:
                            status_text = f"{t} °C | {l} %"
            ...
            if datetime.now().strftime("%Y-%m-%d") != self.persistent_data["last_date"]:
                self.persistent_data.update(
                    {"last_date": datetime.now().strftime("%Y-%m-%d"), "day_kwh": 0.0, "day_cost": 0.0,
                     "day_seconds": 0})
            ...
            SHARED_DATA["sys_w"] = self.cfg_overhead;
            ...
            SHARED_DATA["alert"] = self.persistent_data["day_cost"] > self.cfg_limit;
            SHARED_DATA["price_per_kwh"] = DEFAULT_PRICE;
            SHARED_DATA["daily_limit"] = self.cfg_limit
            ...
            try:
                ...
                if total_w > 500:
                    self.lbl_watts.configure(text_color=COLOR_CRIT)
                elif total_w > 300:
                    self.lbl_watts.configure(text_color=COLOR_WARN)
                else:
                    self.lbl_watts.configure(text_color=COLOR_ACCENT)

                # iGPU UI UPDATE
                if self.card_igpu:
                    self.card_igpu['lbl_val'].configure(text=f"{int(igpu_w)} W")
                    self.card_igpu['lbl_temp'].configure(text=f"{int(igpu_t)} °C  |  {int(igpu_l)} %",
                                                         text_color=self.get_color(igpu_t))
                    self.card_igpu['bar'].set(min(1.0, igpu_w / TDP_IGPU))

                self.card_cpu['lbl_val'].configure(text=f"{int(cpu_w)} W",
                                                   text_color="gray" if is_estimated else COLOR_TEXT_MAIN)
                self.card_cpu['lbl_temp'].configure(text=f"{int(cpu_t)} °C  |  {int(cpu_l)} %",
                                                    text_color=self.get_color(cpu_t))
                ...
                if self.persistent_data["day_cost"] > self.cfg_limit:
                    ...
                    self.lbl_status.configure(text="⚠️ DAILY BUDGET EXCEEDED ⚠️" if int(
                        time.time()) % 2 == 0 else f"Limit: {self.cfg_limit} EGP", text_color=COLOR_CRIT)
                else:
    ...
    def on_close(self):
        self.running = False; self.save_data(); self.destroy()
